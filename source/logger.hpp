...
inline std::string LOG_LINE_STRING = "=======================================";
inline std::string LOG_SIMPLE_LINE_STRING = "---------------------------------------";
...
public:
    ...
    template <typename... Args> constexpr void Log(const std::string& s, Args... args) { string += std::vformat(LOGGER_PREFIX_LOG + s + "\n", std::make_format_args(args...)); }
    template <typename... Args> constexpr void Write(const std::string& s, Args... args) { string += std::vformat(LOGGER_PREFIX_NONE + s + "\n", std::make_format_args(args...)); }

    constexpr void LogLine() { string += LOG_SIMPLE_LINE_STRING + "\n"; }
    ...
    constexpr void LogVectorStats(const List<f32>& v) {
        auto len = v.size();
        if (len == 0) return;
        const auto max = *std::ranges::max_element(v);
        const auto min = *std::ranges::min_element(v);
...
struct Table {
    ...
    template <typename T> void AddColumn(std::string title, Span<T> values) { AddColumnFixed(title, values, title.size() + 1); }
    template <typename T> void AddColumn(std::string title, std::vector<T> values) { AddColumnFixed(title, Span<T>(values), title.size() + 1); }
    template <typename T> void AddColumn(std::string title, List<T> values) { AddColumnFixed(title, Span<T>(values), title.size() + 1); }
