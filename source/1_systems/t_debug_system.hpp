...
private:
    Handle<Node> text_wrapper { B(root).Node(hug).Fill(colors::forest_green).Build() };
...
struct DebugFrame : Frame {
    ...
    NodeComponentPool<DebugNodeComponent> debug_nodes { B(root).Pool<DebugNodeComponent>() };
};
