...
static bool AddFarmToQueue(PlayerEntity player, FARM_TYPE type);
...
void Game::Tick(u32 tick, bool debug) {
    ...
    for (const PlayerEntity player : player_archetype) { (void)AddFarmToQueue(player, RandomKey(data.farm_types)); }
    ...
    const List<String> constructing = Select(player_archetype.construction_queue,  [](const ConstructionQueue& construction_queue) -> String { return {"{} ({})", math::Min(construction_queue.Size(), construction_queue.construction_capacity),  construction_queue.Size()}; } );
...
static bool AddFarmToQueue(const PlayerEntity player, FARM_TYPE type) {
