...
type UnitType = "inf" | "recon";
type UnitSize = "bn" | "plt";
...
interface Frame {
  turn: number; phase: number; units: Unit[];
...
function resolveCombat(atks: Unit[], defs: Unit[], ti: number, attackedTiles: Set<number>): void {
  ...
  for (const d of defs) {
    ...
    dLoss += l;
    if (d.morale <= 20 || d.men < 100) { d.routed = true; addLog(`  X ${d.id} ROUTS (${d.men} flee)`, "result"); }
  }
...
function doMovement(): void {
  ...
  for (const r of units.filter(u => u.type === "recon" && u.side === "atk" && !u.routed)) {
    if (units.some(u => u.side === "def" && u.tile === r.tile && !u.routed)) continue;
    const fw = ADJ[r.tile]
      .filter(t => ROW[t] < ROW[r.tile])
      .sort((a, b) => {
        const oa = units.filter(u => u.tile === a && !u.routed).length;
