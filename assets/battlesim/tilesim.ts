...
interface Tile { index:number; terrain:string; mul:number; icon:string; }
interface LogEntry { text:string; type:"hdr"|"info"|"recon"|"arty"|"combat"|"move"|"result"; }
interface Frame {
  turn:number; phase:number; bats:Battalion[];
...
function esc(s: string) { const el = document.createElement("span"); el.textContent = s; return el.innerHTML; }
function addLog(t: string, ty: LogEntry["type"] = "info") { fullLog.push({ text: t, type: ty }); }

function fp(b: Battalion): number {
  return Math.round(b.rifles / 10 + b.mg + b.mortar * 0.67);
}

function applyCas(b: Battalion, n: number): void {
  const actual = Math.min(n, Math.max(0, b.men));
  b.men -= actual;
  b.rifles = Math.max(0, b.rifles - Math.floor(actual * 0.85));
  if (actual >= 25) b.mg = Math.max(0, b.mg - Math.floor(actual / 70));
  if (actual >= 50 && b.mortar > 0 && d6() <= 2) b.mortar--;
}
...
function simulateAll(): void {
  ...
  while (!over) {
    ...
    if (phase > 4) {
      phase = 1; turn++;
      for (const b of bats) if (!b.routed) b.suppression = Math.max(0, b.suppression - 1);
      addLog(`\n════ TURN ${turn} ════`, "hdr");
    }
    switch (phase) {
      case 1: doRecon(); break;
      case 2: doArtillery(); break;
      case 3: doAssault(); break;
      case 4: doMovement(); break;
    }
    checkEnd();
    steps.push(snap());
  }
    ...
    for (const ti of ADJ[a.tile]) {
      ...
      for (const d of hidden) {
        ...
        if (roll - pen >= 4) {
          ...
          const l = rand(5, 15); applyCas(a, l);
...
function doArtillery(): void {
  addLog("── Artillery  0700–0900 ──", "hdr");

  // ── ATTACKER: 394th Art Bn ──
  if (atkGuns > 0) {
    const tgt = new Set<number>();
    for (const b of bats) if (b.side === "def" && b.revealed && !b.routed) tgt.add(b.tile);
  ...
  if (defGuns > 0) {
    ...
    for (const b of bats) if (b.side === "atk" && !b.routed && !atkTiles.includes(b.tile)) atkTiles.push(b.tile);
...
function doAssault(): void {
  addLog("── Assault  0900–1100 ──", "hdr");
  const plan = new Map<number, Battalion[]>();
  ...
  for (const a of bats.filter(b => b.side === "atk" && !b.routed)) {
    ...
    if (bats.some(b => b.side === "def" && b.tile === a.tile && !b.routed)) targets.push(a.tile);
    if (!targets.length) continue;

    const best = targets.reduce((a2, b2) => {
      const s = (ti: number) => bats.filter(b => b.side === "def" && b.tile === ti && !b.routed).reduce((x, b) => x + fp(b), 0);
  ...
  if (plan.size === 0) { addLog("No engagements.", "info"); return; }
...
function resolveCombat(atks: Battalion[], defs: Battalion[], ti: number, attackedTiles: Set<number>): void {
  const tile = tiles[ti];

  const atkFP = atks.reduce((s, a) => s + fp(a), 0);
  const defFP = defs.reduce((s, d) => s + fp(d), 0);
  if (atkFP <= 0 || defFP <= 0) return;
  ...
  for (const adj of ADJ[ti]) {
    ...
    for (const fd of bats.filter(b => b.side === "def" && b.tile === adj && !b.routed)) {
      ...
      if (contrib > 0) { flankFP += contrib; flankFrom.push(LABELS[adj]); }
    }
  }
  const totalDefFP = defFP + flankFP;
  if (flankFP > 0) addLog(`  flanking fire from ${[...new Set(flankFrom)].join(",")}: +⚔${flankFP}`, "combat");

  // ── entrenchment ──
  const entMul = defs.some(d => d.entrenched) ? 1.3 : 1.0;
  if (entMul > 1) addLog(`  defenders entrenched: ×${entMul} bonus`, "combat");
  ...
  for (const a of atks) {
    ...
    if (a.morale <= 15 || a.men < 80) { a.routed = true; addLog(`  ✗ ${a.id} ROUTS (${a.men} men)`, "result"); }
  }
  ...
  for (const d of defs) {
    ...
    dLoss += l;
    if (d.morale <= 20 || d.men < 100) { d.routed = true; addLog(`  ✗ ${d.id} ROUTS (${d.men} flee)`, "result"); }
  }
...
function doMovement(): void {
  ...
  for (const a of bats.filter(b => b.side === "atk" && !b.routed)) {
    if (bats.some(b => b.side === "def" && b.tile === a.tile && !b.routed)) continue;
  ...
  if (!moved) addLog("No movement.", "info");
}
...
function mkCounters(atks: Battalion[], defs: Battalion[]): string {
  return [...defs, ...atks].map((b, i) => {
    ...
    if (fog) return `<div class="ctr ${side}${fog}" style="${z}">`
...
function renderOOB(fb: Battalion[]): void {
...
function oobRow(b: Battalion, known: boolean): string {
  if (!known)
...
function renderResults(): void {
  const ini: Battalion[] = steps[0].bats;
  const fin: Battalion[] = steps[steps.length - 1].bats;
