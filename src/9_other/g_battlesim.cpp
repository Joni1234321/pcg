...
struct BannerFrame : Frame {
    ...
    Handle<Node> outer { B(frame).Node(fill, hug).Center().Fill(colors::COLOR_CLEAR).Build() };
    ...
    Handle<Node> banner_border { B(card).Node(fill, hug).Fill(colors::COLOR_DARK_SLATE).Padding(3U).Build() };
    Handle<Node> banner { B(banner_border).Node(fill, hug).Fill(colors::COLOR_DARK_GRAY).Build() };
    Handle<Node> banner_left { BuildSide(banner, info.attacker, colors::COLOR_WARM_GRAY) };
    Handle<Node> banner_mid { BuildOutcome(banner) };
    Handle<Node> banner_right { BuildSide(banner, info.defender, colors::COLOR_MAROON) };

    // Details toggle button strip
    Handle<Node> btn_row { B(card).Node(fill, hug).Fill(colors::COLOR_DARK_DARK_BROWN).Padding2({ 10U, 5U }).Build() };
    Handle<Node> btn_label { B(btn_row).Node(hug, hug).Text("▼  Details", FontSizes::small, colors::COLOR_SILVER).Build() };

private:
    Handle<Node> BuildHeader() {
        const Handle<Node> bar = B(card).Node(fill, hug).Fill(colors::COLOR_RUBY_RED).Padding2({ 14U, 10U }).Gap(12U).Build();
        (void)B(bar).Node(hug, hug).Text("⚔", FontSizes::h3, colors::COLOR_WHITE).Build();
        (void)B(bar).Node(fill, hug).Text("COMBAT REPORT", FontSizes::h2, colors::COLOR_WHITE).Build();
        (void)B(bar).Node(hug, hug).Text("⚔", FontSizes::h3, colors::COLOR_WHITE).Right().Build();
        return bar;
    }
    Handle<Node> BuildSide(const Handle<Node> parent, const SideData& side, const Color accent) {
        ...
        const Handle<Node> panel = B(border).Node(fill, hug).Direction(vertical).Gap(0U).Fill(colors::COLOR_DARK_GRAY).Build();

        // CV at top
        const Color cv_color = side.cv_after < side.cv_before ? colors::COLOR_SALMON : colors::COLOR_LIME;
        const Handle<Node> cv = B(panel).Node(fill, hug).Fill(colors::COLOR_DARK_SLATE).Padding2({ 8U, 6U }).Direction(vertical).Gap(0U).Build();
        (void)B(cv).Node(fill, hug).Text("CV", FontSizes::tiny, colors::COLOR_SILVER).Build();
        ...
        (void)B(hdr).Node(fill, hug).Text(side.division, FontSizes::h4, colors::COLOR_WHITE).Build();
        (void)B(hdr).Node(fill, hug).Text(side.role, FontSizes::tiny, colors::COLOR_WHITE_SMOKE).Build();
        return border;
    }
    Handle<Node> BuildOutcome(const Handle<Node> parent) {
        ...
        const Handle<Node> box = B(parent).Node(340U, hug).Direction(vertical).Gap(2U).Fill(colors::COLOR_DARK_DARK_BROWN).Padding2({ 10U, 8U }).Build();
        (void)B(box).Node(fill, hug).Text(attacker_wins ? "AXIS VICTORY" : "SOVIET VICTORY", FontSizes::tiny, colors::COLOR_SILVER).Center().Build();
        (void)B(box).Node(fill, hug).Text(winner, FontSizes::h4, colors::COLOR_DEEP_GOLD).Center().Build();
        (void)B(box).Node(fill, 1U).Fill(colors::COLOR_DARK_SLATE).Build();
        (void)B(box).Node(fill, hug).Text(std::format("{}:{}", info.attacker.cv_after, info.defender.cv_after), FontSizes::title, colors::COLOR_WHITE).Center().Build();
        return box;
    }
};
...
struct DetailsFrame : Frame {
    ...
    Handle<Node> outer { B(frame).Node(fill, hug).Center().Fill(colors::COLOR_CLEAR).Build() };
    Handle<Node> card { B(outer).Node(CARD_WIDTH, hug).Direction(vertical).Gap(0U).Fill(colors::COLOR_DARK_GRAY).Build() };
    Handle<Node> body { B(card).Node(fill, hug).Gap(2U).Fill(colors::COLOR_DARK_SLATE).Padding(2U).Build() };
    Handle<Node> body_left { BuildUnitTable(body, info.attacker, colors::COLOR_WARM_GRAY) };
    ...
    Handle<Node> body_right { BuildUnitTable(body, info.defender, colors::COLOR_MAROON) };
...
private:
    Handle<Node> BuildUnitTable(const Handle<Node> parent, const SideData& side, const Color accent) {
        ...
        const Handle<Node> panel = B(border).Node(fill, hug).Direction(vertical).Gap(0U).Fill(colors::COLOR_WHITE_SMOKE).Build();

        const Handle<Node> label = B(panel).Node(fill, hug).Fill(accent).Padding2({ 6U, 3U }).Build();
        (void)B(label).Node(fill, hug).Text("Unit Strengths", FontSizes::tiny, colors::COLOR_WHITE).Build();

        const Handle<Node> grid = B(panel).Node(fill, hug).Direction(vertical).Fill(colors::COLOR_MID_GRAY).Gap(1U).Build();

        const Handle<Node> header = B(grid).Node(fill, hug).Fill(colors::COLOR_DARK_SLATE).Padding2({ 6U, 3U }).Build();
        (void)B(header).Node(fill, hug).Text("Type", FontSizes::tiny, colors::COLOR_SILVER).Build();
        (void)B(header).Node(COL_NUM, hug).Text("Before", FontSizes::tiny, colors::COLOR_SILVER).Right().Build();
        (void)B(header).Node(COL_NUM, hug).Text("After", FontSizes::tiny, colors::COLOR_SILVER).Right().Build();
        (void)B(header).Node(COL_LOST, hug).Text("Lost", FontSizes::tiny, colors::COLOR_SALMON).Right().Build();

        for (u32 i = 0U; i < side.units.size(); ++i) {
            const UnitRow& unit = side.units[i];
            const Color row_bg = (i % 2U == 0U) ? colors::COLOR_WHITE : colors::COLOR_WHITE_SMOKE;
            const Handle<Node> row = B(grid).Node(fill, hug).Fill(row_bg).Padding2({ 6U, 3U }).Build();
            (void)B(row).Node(fill, hug).Text(unit.name, FontSizes::body, colors::COLOR_BLACK).Build();
            (void)B(row).Node(COL_NUM, hug).Text(std::format("{}", unit.before), FontSizes::body, colors::COLOR_DARK_GRAY).Right().Build();
            (void)B(row).Node(COL_NUM, hug).Text(std::format("{}", unit.after), FontSizes::body, colors::COLOR_DARK_GRAY).Right().Build();
            const u32 lost = unit.before > unit.after ? unit.before - unit.after : 0U;
            (void)B(row).Node(COL_LOST, hug).Text(std::format("-{}", lost), FontSizes::body, colors::COLOR_RUBY_RED).Right().Build();
        }
        return border;
    }
    Handle<Node> BuildRoundsPanel(const Handle<Node> parent) {
        // 2px dark_slate border
        const Handle<Node> border = B(parent).Node(fill, hug).Fill(colors::COLOR_DARK_SLATE).Padding(2U).Direction(vertical).Gap(0U).Build();
        const Handle<Node> panel = B(border).Node(fill, hug).Direction(vertical).Gap(0U).Fill(colors::COLOR_WHITE_SMOKE).Build();

        const Handle<Node> label = B(panel).Node(fill, hug).Fill(colors::COLOR_DARK_SLATE).Padding2({ 6U, 3U }).Build();
        (void)B(label).Node(fill, hug).Text("Combat Rounds", FontSizes::tiny, colors::COLOR_SILVER).Build();

        const Handle<Node> grid = B(panel).Node(fill, hug).Direction(vertical).Fill(colors::COLOR_MID_GRAY).Gap(1U).Build();

        const Handle<Node> rh = B(grid).Node(fill, hug).Fill(colors::COLOR_DARK_SLATE).Padding2({ 6U, 3U }).Gap(6U).Build();
        (void)B(rh).Node(90U, hug).Text("Phase", FontSizes::tiny, colors::COLOR_SILVER).Build();
        (void)B(rh).Node(fill, hug).Text("Attacker", FontSizes::tiny, colors::COLOR_LIGHT_SKY_BLUE).Build();
        (void)B(rh).Node(fill, hug).Text("Defender", FontSizes::tiny, colors::COLOR_SALMON).Build();
        (void)B(rh).Node(100U, hug).Text("Atk -", FontSizes::tiny, colors::COLOR_SILVER).Right().Build();
        (void)B(rh).Node(100U, hug).Text("Def -", FontSizes::tiny, colors::COLOR_SILVER).Right().Build();

        for (u32 i = 0U; i < info.rounds.size(); ++i) {
            const RoundEntry& r = info.rounds[i];
            const Color row_bg = (i % 2U == 0U) ? colors::COLOR_IVORY : colors::COLOR_WHITE;
            const Handle<Node> row = B(grid).Node(fill, hug).Fill(row_bg).Padding2({ 6U, 3U }).Gap(6U).Build();
            round_rows.push_back(row);
            (void)B(row).Node(90U, hug).Text(std::format("#{} {}", i + 1U, r.phase), FontSizes::small, colors::COLOR_DEEP_PURPLE).Build();
            (void)B(row).Node(fill, hug).Text(r.attacker_action, FontSizes::small, colors::COLOR_NAVY).Build();
            (void)B(row).Node(fill, hug).Text(r.defender_action, FontSizes::small, colors::COLOR_MAROON).Build();
            (void)B(row).Node(100U, hug).Text(std::format("{}", r.attacker_losses), FontSizes::small, colors::COLOR_RUBY_RED).Right().Build();
            (void)B(row).Node(100U, hug).Text(std::format("{}", r.defender_losses), FontSizes::small, colors::COLOR_RUBY_RED).Right().Build();
        }
        return border;
    }
};
...
struct RoundPopupFrame : Frame {
    ...
    Handle<Node> outer { B(frame).Node(fill, fill).Center().Fill(colors::COLOR_CLEAR).Build() };
    Handle<Node> card { B(outer).Node(760U, hug).Direction(vertical).Gap(0U).Build() };

    Handle<Node> hdr { B(card).Node(fill, hug).Fill(colors::COLOR_DARK_SLATE).Padding2({ 14U, 10U }).Gap(10U).Build() };
    Handle<Node> phase_lbl { B(hdr).Node(fill, hug).Text("Round", FontSizes::h3, colors::COLOR_SILVER).Build() };

    Handle<Node> body { B(card).Node(fill, hug).Direction(vertical).Gap(0U).Fill(colors::COLOR_DARK_GRAY).Padding(14U).Build() };

    Handle<Node> atk_hdr { B(body).Node(fill, hug).Text("Attacker Action", FontSizes::tiny, colors::COLOR_WARM_GRAY).Build() };
    Handle<Node> atk_txt { B(body).Node(fill, hug).Text("", FontSizes::body, colors::COLOR_WHITE_SMOKE).Padding2({ 0U, 5U }).Build() };

    Handle<Node> div { B(body).Node(fill, 1U).Fill(colors::COLOR_DARK_SLATE).Padding2({ 0U, 4U }).Build() };

    Handle<Node> def_hdr { B(body).Node(fill, hug).Text("Defender Action", FontSizes::tiny, colors::COLOR_SALMON).Build() };
    Handle<Node> def_txt { B(body).Node(fill, hug).Text("", FontSizes::body, colors::COLOR_WHITE_SMOKE).Padding2({ 0U, 5U }).Build() };

    Handle<Node> loss_row { B(body).Node(fill, hug).Gap(8U).Padding2({ 0U, 10U }).Build() };
    Handle<Node> atk_loss { B(loss_row).Node(fill, hug).Text("", FontSizes::h2, colors::COLOR_WARM_GRAY).Build() };
    Handle<Node> def_loss { B(loss_row).Node(fill, hug).Text("", FontSizes::h2, colors::COLOR_RUBY_RED).Right().Build() };

    Handle<Node> hint { B(card).Node(fill, hug).Fill(colors::COLOR_DARK_DARK_BROWN).Padding2({ 8U, 5U }).Text("click anywhere to dismiss", FontSizes::small, colors::COLOR_SILVER).Center().Build() };
...
struct StatusBarFrame : Frame {
    ...
    Handle<Node> panel { B(outer).Node(PANEL_WIDTH, hug).Direction(vertical).Gap(1U).Fill(colors::COLOR_DARK_SLATE).Padding(2U).Build() };
...
private:
    Array<StatusEntry, NUM_ENTRIES> BuildEntries() {
        ...
        for (u32 i = 0U; i < NUM_ENTRIES; ++i) {
            const Color row_bg = (i % 2U == 0U) ? colors::COLOR_DARK_DARK_BROWN : colors::COLOR_DARK_GRAY;
            const Handle<Node> row = B(panel).Node(fill, hug).Direction(horizontal).Gap(6U).Fill(row_bg).Padding2({ 6U, 4U }).Build();
            const Handle<Node> bar_bg = B(row).Node(BAR_WIDTH, BAR_HEIGHT).Fill(colors::COLOR_DARK_SLATE).Build();
            const Handle<Node> bar_fill = B(bar_bg).Node(0U, BAR_HEIGHT).Fill(colors::COLOR_FOREST_GREEN).Build();
            const Handle<Node> pct = B(row).Node(PCT_COL, hug).Text("0%", FontSizes::tiny, colors::COLOR_SILVER).Right().Build();
            const Handle<Node> msg = B(row).Node(fill, hug).Text("Idle", FontSizes::tiny, colors::COLOR_SILVER).Build();
