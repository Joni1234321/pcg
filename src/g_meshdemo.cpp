...
struct GlyphAtlas {
    ...
    bool Build(SDL_Renderer* r, const char* font_path, float pt) {
        ...
        SDL_FillSurfaceRect(atlas, &wr,
            SDL_MapSurfaceRGBA(atlas, 255, 255, 255, 255));
        white_uv = { 1.0f / total_w, 1.0f / max_h,
                     1.0f / total_w, 1.0f / max_h }; // 1px inside the block
...
public:
    ...
    void MakeSolidQuad(SDL_FRect dst, SDL_FColor col, const GlyphAtlas& atlas) {
        MakeQuad(dst, atlas.white_uv, col, atlas.texture);
    }
    ...
    void MakeHex(SDL_FPoint center, float radius, SDL_FColor col, const GlyphAtlas& atlas) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            verts_.push_back({
                { center.x + SDL_cosf(a) * radius, center.y + SDL_sinf(a) * radius },
                col, uv });
        }
        for (int i = 0; i < 6; ++i)
            PushTri(base, base + 1 + i, base + 1 + ((i + 1) % 6));
    }

    void MakeText(std::string_view s, SDL_FPoint pos, const GlyphAtlas& atlas, SDL_FColor col) {
        MakeTextScaled(s, pos, atlas, col, 1.f);
    }
    ...
    void MakeTextScaled(std::string_view s, SDL_FPoint pos, const GlyphAtlas& atlas,
                        SDL_FColor col, float scale) {
        SetTexture(atlas.texture);
        float pen_x = pos.x;
    ...
    void MakeTextAtPixelHeight(std::string_view s, SDL_FPoint pos,
                               const GlyphAtlas& atlas, SDL_FColor col,
                               float target_px) {
    ...
    void MakeLabel(std::string_view text, SDL_FPoint pos, const GlyphAtlas& atlas,
                   SDL_FColor text_col, SDL_FColor bg_col,
                   float text_px, float pad_px = 6.f) {
        ...
        MakeTextScaled(text,
                       { pos.x + pad_px, pos.y + pad_px },
                       atlas, text_col, scale);
    }

    void Flush(SDL_Renderer* r) {
        for (auto& b : batches_) {
            if (b.idx_count == 0) continue;
            SDL_RenderGeometry(r, b.tex,
                verts_.data(), (int)verts_.size(),
                indices_.data() + b.idx_offset, b.idx_count);
        }
        ++flush_count_;
    }
...
// ---------------------------------------------------------------------------
static void DrawCounter(MeshBuilder& mb, const GlyphAtlas& atlas,
                        SDL_FPoint pos, int value, std::string_view label) {
    ...
    mb.MakeTextAtPixelHeight(label, { pos.x + 12.f, pos.y + 22.f },
                             atlas, TEXT,   16.f);
    mb.MakeTextAtPixelHeight(num,   { pos.x + 12.f, pos.y + 44.f },
                             atlas, ACCENT, 32.f);
}
...
static SDL_Texture* MakeCheckerTexture(SDL_Renderer* r) {
    ...
    for (int y = 0; y < N; ++y) {
        for (int x = 0; x < N; ++x) {
            ...
            Uint8 v     = dark ? 60 : 200;
            px[y * (s->pitch / 4) + x] =
                SDL_MapSurfaceRGBA(s, v, v, (Uint8)(v / 2 + 60), 255);
        }
    }
...
int main(int /*argc*/, char* /*argv*/[]) {
    ...
    if (!TTF_Init()) {
        SDL_Log("TTF_Init: %s", SDL_GetError()); return 1;
    }

    SDL_Window*   win  = nullptr;
    SDL_Renderer* rdr  = nullptr;
    if (!SDL_CreateWindowAndRenderer("SDL3 Mesh Batching Demo",
                                     900, 600, 0, &win, &rdr)) {
    ...
    if (!atlas.Build(rdr,
            "assets/Courier_Prime/CourierPrime-Bold.ttf", 48.f)) {
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&ev)) {
            ...
            if (ev.type == SDL_EVENT_KEY_DOWN) {
                ...
                if (ev.key.key == SDLK_EQUALS || ev.key.key == SDLK_PLUS)
                    zoom *= 1.1f;
                if (ev.key.key == SDLK_MINUS) zoom /= 1.1f;
                if (ev.key.key == SDLK_0)   { zoom = 1.f; pan = { 0, 0 }; }
        ...
        mb.MakeSolidQuad({ 20, 20, 860, 60 },
                         { 0.15f, 0.18f, 0.24f, 1.f }, atlas);
        mb.MakeTextAtPixelHeight(
            "SDL3 mesh batching demo  --  wheel/+- zoom, WASD or drag to pan",
            { 36, 30 }, atlas, { 0.95f, 0.95f, 0.95f, 1.f }, 18.f);
        ...
        for (int i = 0; i < 24; ++i) {
            ...
            SDL_FColor c {
                0.5f + 0.5f * SDL_sinf(t_sec + i * 0.3f),
                0.5f + 0.5f * SDL_sinf(t_sec + i * 0.3f + 2.f),
                0.5f + 0.5f * SDL_sinf(t_sec + i * 0.3f + 4.f),
                1.f
            };
            SDL_FPoint world_pos {
                world_origin.x + local_x * zoom,
                world_origin.y + local_y * zoom
            };
        ...
        mb.MakeLabel("world-space (scales with zoom)",
            { world_origin.x - 140.f * zoom, world_origin.y + 28.f * zoom },
            atlas,
            { 0.95f, 0.95f, 1.0f, 1.f },
            { 0.10f, 0.14f, 0.22f, 0.85f },
            14.f * zoom, 6.f * zoom);
        ...
        mb.MakeSolidQuad({ 760, 200, 120, 90 },
                         { 0.15f, 0.18f, 0.24f, 1.f }, atlas);
        mb.MakeTextAtPixelHeight("TINY",  { 770, 206 }, atlas,
                                 { 0.95f, 0.95f, 0.95f, 1.f }, 10.f);
        mb.MakeTextAtPixelHeight("SMALL", { 770, 222 }, atlas,
                                 { 0.95f, 0.95f, 0.95f, 1.f }, 14.f);
        mb.MakeTextAtPixelHeight("BIG",   { 770, 244 }, atlas,
                                 { 1.0f, 0.78f, 0.20f, 1.f }, 32.f);

        // ---- BATCH 2: a second texture forces a new SDL_RenderGeometry -----
        mb.MakeQuad({ 40, 320, 200, 200 },
                    { 0.f, 0.f, 1.f, 1.f },
                    { 1.f, 1.f, 1.f, 1.f }, checker);

        // ---- BATCH 3: back to the atlas (text overlay on the checker) ------
        mb.MakeText("texture swap = new batch",
                    { 50, 330 }, atlas, { 1.f, 1.f, 0.2f, 1.f });

        // ---- BATCH 1 continued logically, but split because of batch 2 ----
        // status text (screen-space, fixed size)
        char stats[200];
        std::snprintf(stats, sizeof(stats),
            "zoom:%.2fx  pan:(%.0f,%.0f)  batches:%d  verts:%d  frame:%.2f ms",
            zoom, pan.x, pan.y, mb.BatchCount(), mb.VertexCount(), frame_avg);
        mb.MakeTextAtPixelHeight(stats, { 40, 560 }, atlas,
            { 0.7f, 0.85f, 1.f, 1.f }, 14.f);
