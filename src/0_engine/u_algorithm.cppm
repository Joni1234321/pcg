...
template <typename Iterator1, typename Iterator2, typename OutputIterator, typename BinaryOperation> constexpr OutputIterator TransformLocal(Iterator1 first1, Iterator1 last1, Iterator2 first2, OutputIterator result, BinaryOperation binary_operation) {
    while (first1 != last1) {
...
template <typename Container1, typename Container2, typename OutputContainer, typename BinaryOperation> constexpr OutputContainer::iterator Transform(const Container1& container1, const Container2& container2, OutputContainer& out, BinaryOperation binary_operation) {
    return TransformLocal(std::begin(container1), std::end(container1), std::begin(container2), std::begin(out), binary_operation);
}

template <typename Container, typename OutputContainer, typename UnaryOperation> constexpr OutputContainer Transform(Container iterator, OutputContainer out, UnaryOperation binary_operation) { return TransformLocal(std::begin(iterator), std::end(iterator), std::begin(out), binary_operation); }
...
public:
    template <class T, class V, class P> [[nodiscard]] constexpr HandleOptional<T> operator()(const HandleList<T>& handleList, const V& value, P projection) const {
        const auto& iterator = std::ranges::find(handleList, value, projection);
        if (iterator == handleList.end()) { return HandleOptional<T> { std::nullopt }; }
