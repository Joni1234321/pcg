...
private:
    Handle<Node> text_wrapper { B(root).Node(hug).Fill(colors::COLOR_CLEAR).Build() };
    Handle<Node> text { B(text_wrapper).Node(hug).Text(FontSizes::small, colors::COLOR_BLACK).Build() };
...
struct TickFrame : Frame {
    ...
    Handle<Node> ticks { B(root).Node(hug).Padding2({ 0U, 4U }).Text(FontSizes::body, colors::COLOR_RADIANT_ORANGE).Build() };
...
struct DebugFrame : Frame {
    ...
    Handle<Node> root { B(frame).Node(hug).Padding2({ 10U, 30U }).Fill(colors::COLOR_BEIGE).Direction(vertical).Build() };
    Handle<Node> hovered_label { B(root).Node(hug).Text(FontSizes::body, colors::COLOR_BLACK).Build() };

    Handle<Node> box { B(root).Node(hug).Fill(colors::COLOR_ORANGE).Padding(4U).Direction(vertical).Center().Build() };

    Handle<Node> padding_top { B(box).Node(hug).Text(FontSizes::body, colors::COLOR_BLACK).Build() };
    Handle<Node> center { B(box).Node(hug).Gap(4U).Build() };
    Handle<Node> padding_bottom { B(box).Node(hug).Text(FontSizes::body, colors::COLOR_BLACK).Center().Build() };

    Handle<Node> padding_left { B(center).Node(hug).Text(FontSizes::body, colors::COLOR_BLACK).Build() };
    Handle<Node> inner { B(center).Node(hug).Fill(colors::COLOR_BEIGE).Build() };
    Handle<Node> padding_right { B(center).Node(hug).Text(FontSizes::body, colors::COLOR_BLACK).Build() };

    Handle<Node> size { B(inner).Node(hug).Text(FontSizes::body, colors::COLOR_BLACK).Build() };
