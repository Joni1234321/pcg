...
inline void RenderCounters(const List<Counter>& counters) {
    ...
    for (const Counter& counter : counters) {
        ...
        for (i32 i = static_cast<i32>(counter.colors.size()) - 1; i >= 0; i--) {
            ...
            constexpr f32 STACK_OFFSET = 1.0F / 15.0F;
            constexpr f32 SHADOW_OFFSET = STACK_OFFSET * 0.25F;
            constexpr f32 BORDER_THICKNESS = STACK_OFFSET * 0.25F;
            ...
            const SDL_FRect area_shadow {
                .x = area.x + SHADOW_OFFSET * area_counter.w,
                .y = area.y + SHADOW_OFFSET * area_counter.w,
                .w = area.w,
                .h = area.h,
            };
            ...
            // border
            const SDL_Color border_color = counter.selected ? colors::yellow : colors::black;
            ...
            // background
            const f32 border_width = BORDER_THICKNESS * area_counter.w;
